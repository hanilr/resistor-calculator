...
def make_band_select(pos_x, pos_y):
    four_band_button = Button(bd=0, bg=bg_button, fg=fg_color, text="Clear", command=lambda: no_window_left("six_band.py"))
    ...
    six_band_button = Button(bd=0, bg=bg_button, fg=fg_color, text="Five Band", command=lambda: no_window_left("five_band.py"))
...
temp_hot = StringVar()
color_temp_hot = Entry(text="", textvariable=temp_hot)
color_temp_hot.pack()
color_temp_hot.place(x=510, y=290, width=10)
...
hot_color = Canvas(main_screen, height=150, width=20, bd=0, bg=bg_canvas)
hot_color.pack()
hot_color.place(x=330, y=20)
...
hot_menu = Menubutton(main_screen, bg=bg_menu, fg=fg_color, text="°C")
hot_menu.pack()
hot_menu.place(x=320, y=190, height=20, width=50)
...
hot_var_result = StringVar()
hot_result_var = Entry(main_screen, bd=0, bg=bg_entry, fg=fg_color, font=("", 10), textvariable=hot_var_result, justify=CENTER)
hot_result_var.pack()
hot_result_var.place(x=400, y=170, height=20, width=120)
hot_result_var.insert(0, "Δ 1 °C")
...
result_var.pack()
result_var.place(x=380, y=200, height=60, width=160)
...
hot_menu.menu = Menu(hot_menu)
hot_menu["menu"] = hot_menu.menu
hot_menu.menu.add_command(label="Brown", command=lambda: make_color_hot(hex_brown))
hot_menu.menu.add_command(label="Red", command=lambda: make_color_hot(hex_red))
hot_menu.menu.add_command(label="Orange", command=lambda: make_color_hot(hex_orange))
hot_menu.menu.add_command(label="Yellow", command=lambda: make_color_hot(hex_yellow))
hot_menu.menu.add_command(label="Blue", command=lambda: make_color_hot(hex_blue))
hot_menu.menu.add_command(label="Purple", command=lambda: make_color_hot(hex_purple))
# -------------------- #
...
def make_color_hot(color_hot):
    hot_color = Canvas(main_screen, height=150, width=20, bd=0, bg=color_hot)
    hot_color.pack()
    hot_color.place(x=330, y=20)
    color_temp_hot.delete(0, END)
    color_temp_hot.insert(0, color_hot)
...
def calc_res():
    ...
    int_var_hot = calc_check(temp_hot.get(), 20)
    temp_color_result = (int_var1*10 + int_var2*10 + int_var_shalf) * int_var3
    result_hot_var = int_var_hot*temp_color_result/1000000
    ...
    hot_result_var.delete(0, END)
    hot_result_var.insert(0, result_hot_var/100)
# --------------------- #
