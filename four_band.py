...
def make_band_select(pos_x, pos_y):
    four_band_button = Button(bd=0, bg=bg_button, fg=fg_color, text="Clear", command=lambda: no_window_left("four_band.py"))
    four_band_button.pack()
    four_band_button.place(x=pos_x, y=pos_y, height=40, width=140)

    five_band_button = Button(bd=0, bg=bg_button, fg=fg_color, text="Five Band", command=lambda: no_window_left("five_band.py"))
...
color_temp_var3.place(x=480, y=290, width=10)

temp_var4 = StringVar()
color_temp_var4 = Entry(text="", textvariable=temp_var4)
color_temp_var4.pack()
color_temp_var4.place(x=490, y=290, width=10)
...
result_var.pack()
result_var.place(x=270, y=200, height=60, width=160)
...
def calc_res():
    ...
    temp_color_result = (int_var1 + int_var2) * int_var3
